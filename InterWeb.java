 ...
 */
@DesignerComponent(
    description = "Non-visible component that provides functions for HTTP GET, POST, PUT, and DELETE requests. With Extended Functions.<br><a href="https://github.com/hrichiksite/InterWeb">Source Code</a>",
...
@UsesPermissions(permissionNames = "android.permission.INTERNET," +
  ...
  public final class InterWeb extends AndroidNonvisibleComponent{
...
public class InterWeb extends AndroidNonvisibleComponent{
  ...
  private static class InvalidRequestHeadersException extends Exception {
     ...
     * ErrorMessages.ERROR_InterWeb_REQUEST_HEADER_NOT_LIST
     * ErrorMessages.ERROR_InterWeb_REQUEST_HEADER_NOT_TWO_ELEMENTS
     */
  ...
  static class BuildRequestDataException extends Exception {
    /*
     * errorNumber could be:
     * ErrorMessages.ERROR_InterWeb_BUILD_REQUEST_DATA_NOT_LIST
     * ErrorMessages.ERROR_InterWeb_BUILD_REQUEST_DATA_NOT_TWO_ELEMENTS
     */
    final int errorNumber;
    final int index;         // the index of the invalid header
  ...
  /**
   * The CapturedProperties class captures the current property values from a InterWeb component before
  ...
  private static class CapturedProperties {
    ...
    CapturedProperties(InterWeb InterWeb) throws MalformedURLException, InvalidRequestHeadersException {
      urlString = InterWeb.urlString;
      url = new URL(urlString);
      allowCookies = InterWeb.allowCookies;
      saveResponse = InterWeb.saveResponse;
      responseFileName = InterWeb.responseFileName;
      timeout = InterWeb.timeout;
      requestHeaders = processRequestHeaders(InterWeb.requestHeaders);

      Map<String, List<String>> cookiesTemp = null;
      if (allowCookies && InterWeb.cookieHandler != null) {
        try {
          cookiesTemp = InterWeb.cookieHandler.get(url.toURI(), requestHeaders);
  ...
  private static final String LOG_TAG = "InterWeb";
  ...
  /**
   * Creates a new InterWeb component.
   *
   * @param container the Form that this component is contained in.
   */
  public InterWeb(ComponentContainer container) {
  ...
  /**
   * The URL for the InterWeb request.
   *
   * @return the URL
   */
  @SimpleProperty(category = PropertyCategory.BEHAVIOR,
      description = "The URL for the InterWeb request.")
  public String Url() {
    return urlString;
  }
 ...
 /**
  * Sets Cookie For The Folowing URL
  */
...
public void SetCookies(String url,String cookieString){
   ...
   */
  @SimpleProperty(category = PropertyCategory.BEHAVIOR,
      description = "The number of milliseconds that a InterWeb request will wait for a response before giving up. " +
  ...
  public void Timeout(int timeout) {
    if (timeout < 0){
      throw new IllegalArgumentError("InterWeb Timeout must be a non-negative integer.");
    }
    this.timeout = timeout;
  }

  @SimpleFunction(description = "Clears all cookies for this InterWeb component.")
  ...
  public void Get() {
    ...
    final CapturedProperties InterWebProps = capturePropertyValues(METHOD);
    if (InterWebProps == null) {
    ...
    AsynchUtil.runAsynchronously(new Runnable() {
      @Override
      public void run() {
        performRequest(InterWebProps, null, null, "GET", METHOD);
      }
    });
  }
  ...
  public void PostFile(final String path) {
    ...
    AsynchUtil.runAsynchronously(new Runnable() {
      @Override
      public void run() {
        performRequest(InterWebProps, null, path, "POST", METHOD);
      }
    });
  }
  ...
  public void PutFile(final String path) {
    ...
    AsynchUtil.runAsynchronously(new Runnable() {
      @Override
      public void run() {
        performRequest(InterWebProps, null, path, "PUT", METHOD);
      }
    });
  }
  ...
  public void Delete() {
    ...
    AsynchUtil.runAsynchronously(new Runnable() {
      @Override
      public void run() {
        performRequest(InterWebProps, null, null, "DELETE", METHOD);
      }
    });
  }
  ...
  private void requestTextImpl(final String text, final String encoding,
    ...
    // Capture property values before running asynchronously.
    final CapturedProperties InterWebProps = capturePropertyValues(functionName);
    if (InterWebProps == null) {
      // capturePropertyValues has already called form.dispatchErrorOccurredEvent
      return;
    }
    ...
    AsynchUtil.runAsynchronously(new Runnable() {
      ...
      public void run() {
        ...
        performRequest(InterWebProps, requestData, null, httpVerb, functionName);
      }
    });
  }
   ...
   * list `(A B)`.
   *
   *   Use the method [JsonTextDecodeWithDictionaries](#InterWeb.JsonTextDecodeWithDictionaries) if you
   ...
   * and the InterWeb component will return the empty string.
   *
  ...
  // HTML for the component documentation on the InterWeb.  It's too long for a tooltip, anyway.
   ...
   *
   * @param InterWebProps the captured property values needed for the request
   ...
   * @throws IOException
   */
  private void performRequest(final CapturedProperties InterWebProps, final byte[] postData,
    ...
    if (saveResponse & !havePermission) {
      final InterWeb me = this;
      form.askPermission(new BulkPermissionRequest(this, "InterWeb",
          ...
          public void onGranted() {
            ...
            AsynchUtil.runAsynchronously(new Runnable() {
                @Override
                public void run() {
                  me.performRequest(InterWebProps, postData, postFile, httpVerb, method);
                }
              });
          }
        });
    ...
    try {
      // Open the connection.
      HttpURLConnection connection = openConnection(InterWebProps, httpVerb);
      if (connection != null) {
        try {
          if (postData != null) {
          ...
          if (saveResponse) {
            final String path = saveResponseContent(connection, InterWebProps.responseFileName,
              responseType);
            ...
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                  GotFile(InterWebProps.urlString, responseCode, responseType, path);
                }
              });
          } else {
            ...
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                  GotText(InterWebProps.urlString, responseCode, responseType, responseContent);
                }
              });
          }
          ...
          activity.runOnUiThread(new Runnable() {
              @Override
              public void run() {
                TimedOut(InterWebProps.urlString);
              }
            });
      ...
      form.dispatchPermissionDeniedEvent(InterWeb.this, method, e);
      ...
      form.dispatchErrorOccurredEvent(InterWeb.this, method,
        message, InterWebProps.urlString);
    }
  }
   ...
   * @param InterWebProps the properties of the connection, set as properties in the component
   ...
   */
  private static HttpURLConnection openConnection(CapturedProperties InterWebProps, String httpVerb)
      throws IOException, ClassCastException, ProtocolException {

    HttpURLConnection connection = (HttpURLConnection) InterWebProps.url.openConnection();
    connection.setConnectTimeout(InterWebProps.timeout);
    connection.setReadTimeout(InterWebProps.timeout);
    ...
    // Request Headers
    for (Map.Entry<String, List<String>> header : InterWebProps.requestHeaders.entrySet()) {
    ...
    // Cookies
    if (InterWebProps.cookies != null) {
      for (Map.Entry<String, List<String>> cookie : InterWebProps.cookies.entrySet()) {
